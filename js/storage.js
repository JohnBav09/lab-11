...
var increment_item_clicks = function(item_image_id){
  if (item_image_id === 'left_pic_img') {
    currenty_displayed_left_pic.clicked_on_count++;
  } else if (item_image_id === 'right_pic_img') {
    currenty_displayed_right_pic.clicked_on_count++;
  }
};
...
var handle_click_on_item = function (event) {
  if(event.target.tagName === 'IMG'){
    ...
    if(totes_items <=0){
      ...
      var stringy_Busmall = JSON.stringify(the_catalog);
      localStorage.setItem('all_items', stringy_Busmall);
...
if(localStorage.getItem('all_items')){
  var stringy_Busmall = localStorage.getItem('all_items');
  the_catalog = JSON.parse(stringy_Busmall);
  console.log(`retrieved ${the_catalog.length} items from local storage`);
} else {
  ...
  console.log('created brand new items');
}
